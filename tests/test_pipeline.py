...
def test_itp_to_struct_ganglioside_head_up(tmp_path):
    """A ganglioside (large branched sugar head plus two acyl tails) must orient
    with the sugar head up and the acyl tails down. Rooting at an inner sugar
    would let a sialic acid tip rank among the deepest beads and be placed into
    the membrane; rooting at the outermost head bead avoids this."""
    itp = tmp_path / "DPG3.itp"
    itp.write_text(
        "[ moleculetype ]\nDPG3 1\n[ atoms ]\n"
        " 1 SP1 1 DPG3 B1  1 0\n 2 SP1 1 DPG3 B2  2 0\n 3 SP1 1 DPG3 B3  3 0\n"
        " 4 SP1 1 DPG3 S1  4 0\n 5 SP1 1 DPG3 S2  5 0\n 6 SP1 1 DPG3 S3  6 0\n"
        " 7 P1  1 DPG3 AM1 7 0\n 8 P1  1 DPG3 AM2 8 0\n"
        " 9 C1  1 DPG3 T1A 9 0\n10 C1  1 DPG3 T2A 10 0\n11 C1  1 DPG3 T3A 11 0\n"
        "12 C1  1 DPG3 T4A 12 0\n13 C1  1 DPG3 C1B 13 0\n14 C1  1 DPG3 C2B 14 0\n"
        "15 C1  1 DPG3 C3B 15 0\n[ bonds ]\n"
        " 1 2 1 0.4 1250\n 2 3 1 0.4 1250\n 3 4 1 0.4 1250\n 4 5 1 0.4 1250\n"
        " 5 6 1 0.4 1250\n 1 7 1 0.4 1250\n 7 8 1 0.4 1250\n 8 9 1 0.4 1250\n"
        " 9 10 1 0.4 1250\n10 11 1 0.4 1250\n11 12 1 0.4 1250\n 7 13 1 0.4 1250\n"
        "13 14 1 0.4 1250\n14 15 1 0.4 1250\n")
    out = tmp_path / "DPG3.gro"
    run("itp_to_struct.py", "--itp", itp, "--mol", "DPG3", "--out", out)
    L = out.read_text().splitlines()
    body = L[2:2 + int(L[1])]
    z = {b[10:15].strip(): float(b[36:44]) for b in body}
    # whole sugar head above the midplane
    for s in ("B1", "B2", "B3", "S1", "S2", "S3"):
        assert z[s] > 0, "%s should be up" % s
...
def _gro_pair(path, rows, box=(6.0, 6.0, 6.0)):
    """rows: (resid, resname, atomname, x, y, z)."""
    body = ["%5d%-5s%5s%5d%8.3f%8.3f%8.3f" % (r[0], r[1], r[2], i + 1,
                                              r[3], r[4], r[5])
...
def test_two_water_beads_on_top_of_each_other_do_not_stop_a_build(tmp_path):
    """A pair of water beads packed closer than the threshold is separated by
    the first steps of the minimization. The number of water beads grows with
    the box, so a large system carries such a pair almost every time, and
    stopping on it makes every large build need an override."""
    gro = tmp_path / "w.gro"
    _gro_pair(gro, [(1, "W", "W", 1.000, 1.0, 1.0),
                    (2, "W", "W", 1.080, 1.0, 1.0),
                    (3, "DLPC", "NC3", 3.000, 3.0, 3.0),
                    (4, "DLPC", "NC3", 3.500, 3.0, 3.0)])
